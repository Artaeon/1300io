...
import { authenticateToken, canAccessOrg } from '../middleware/auth';
import { pdfLimiter } from '../middleware/rateLimiters';
import { uploadDir } from '../middleware/uploadHandler';
import { idParamSchema, validateParams } from '../schemas';
  ...
  asyncHandler(async (req, res) => {
    ...
    const inspection = await prisma.inspection.findUnique({
      ...
      include: {
        property: true,
        results: {
          include: { checklist_item: { include: { category: true } } },
        },
      },
    });
    ...
    const rows = [['Kategorie', 'Pruefpunkt', 'Status', 'Kommentar', 'Foto-URL'].join(',')];
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=Pruefbericht-${inspection.id}.csv`,
    );
    res.send('\uFEFF' + csv);
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    doc.fontSize(14).font('Helvetica').text('nach ONORM B 1300', 50, 65, { align: 'center' });
    doc.fontSize(10).text(`Bericht #${inspection.id}`, 50, 90, { align: 'center' });
    ...
    doc.text(
      `Status: ${inspection.status === 'COMPLETED' ? 'Abgeschlossen' : 'Entwurf'}`,
      350,
      infoY + 58,
    );
    ...
    type ResultRow = typeof inspection.results[number];
    const resultsByCategory: Record<string, ResultRow[]> = {};
    for (const result of inspection.results) {
      if (result.checklist_item?.category) {
    ...
    for (const [categoryName, results] of Object.entries(resultsByCategory)) {
      if (currentY > 700) {
        doc.addPage();
        currentY = 50;
      }
      ...
      doc.fillColor('white').fontSize(10).font('Helvetica-Bold').text(categoryName.toUpperCase(), 55, currentY + 6);
      currentY += 22;
      ...
      results.forEach((result, index) => {
        ...
        doc.fillColor(statusColor).font('Helvetica-Bold').text(statusText, 420, currentY + 4, { width: 80 });
    ...
    if (defects.length > 0) {
      ...
      doc.fillColor('white').fontSize(20).font('Helvetica-Bold').text('MAENGELBERICHT', 50, 20, { align: 'center' });
      doc.fontSize(12).font('Helvetica').text(`${defects.length} Mangel/Maengel dokumentiert`, 50, 42, { align: 'center' });
      ...
      defects.forEach((defect, index) => {
        ...
        doc.fontSize(12).font('Helvetica-Bold').fillColor('#721c24').text(`Mangel #${index + 1}`, 50, defectY);
        defectY += 18;

        doc.fontSize(10).font('Helvetica').fillColor('black').text(`Kategorie: ${defect.checklist_item.category.name}`, 50, defectY);
        defectY += 15;
        ...
        // Photo: path traversal protection — only allow files in uploadDir
        if (defect.photo_url) {
          ...
          if (safePath.startsWith(uploadDir) && fs.existsSync(safePath)) {
            try {
              ...
              defectY += 200;
              doc.fontSize(8).fillColor('#666').text('Foto des Mangels', 50, defectY);
              defectY += 15;
            } catch (imgError) {
              logger.error('Error embedding image in PDF', {
                inspectionId: inspection.id,
                error: imgError instanceof Error ? imgError.message : String(imgError),
                requestId: req.id,
              });
              doc.fontSize(10).fillColor('#999').text('[Bild konnte nicht geladen werden]', 50, defectY);
              defectY += 15;
            }
          }
        }
    ...
    doc.fontSize(8).fillColor('#999').text(
      `Generiert am ${new Date().toLocaleString('de-AT')} | 1300.io v1.0`,
      50,
      750,
      { align: 'center' },
    );
